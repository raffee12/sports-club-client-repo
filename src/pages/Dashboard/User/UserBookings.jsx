...
export default function UserBookings() {
  ...
  // ✅ Show SweetAlert on becoming member
  useEffect(() => {
    if (role === "member") {
      Swal.fire({
        icon: "success",
  ...
  const handleCancel = async (bookingId) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        await refetch();
        await refetchRole(); // 🔄 refresh role in case it changes
      ...
      {bookings.some((b) => b.status === "approved") && role === "user" && (
        <div className="text-center">
          <button
            ...
            className="btn btn-success text-white mt-2 hover:scale-105 transition-all duration-300 cursor-pointer"
          >
            🎉 Join the Club Now
          </button>
        </div>
                ...
                <div className="mt-4 flex justify-between items-center">
                  ...
                  <button
                    ...
                    className={`btn btn-xs text-white flex items-center gap-1 ${
                      cancelingId === booking._id ? "btn-disabled" : "btn-error"
                    }`}
                  >
                    <FiTrash2 />
